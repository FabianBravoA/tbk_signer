...
export default (webpayKey, xml) => {
  ...
  sig.validateSignatureValue = () => {
    ...
    let signedInfoCanon = sig.getCanonXml([this.canonicalizationAlgorithm], signedInfo[0]);
    ...
    const res = signer.verifySignature(signedInfoCanon, this.signingKey, this.signatureValue);
    if (!res) this.validationErrors.push(`invalid signature: the signature value ${sig.signatureValue} is incorrect`);
    return res;
  };
