  ...
  return Promise.resolve(createPrivateKey(privatePEM))
    ...
    .then(
      (publicKey) => {
        ...
        tbkSecurity.publicP12PEM = publicKeyPEM.replace('-----BEGIN CERTIFICATE-----', '').replace('-----END CERTIFICATE-----', '').replace(/(\r\n|\n|\r)/gm, '');
        tbkSecurity.signer.signingKey = tbkSecurity.privateKey.export({
          type: 'pkcs1',
          format: 'pem',
        });
