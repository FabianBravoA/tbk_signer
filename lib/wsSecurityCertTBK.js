...
export default (privatePEM, publicP12PEM) => {
  ...
  tbkSecurity.publicP12PEM = publicP12PEM.replace('-----BEGIN CERTIFICATE-----', '').replace('-----END CERTIFICATE-----', '').replace(/(\r\n|\n|\r)/gm, '');
    ...
    .then(
      (pemData) => {
        ...
        if (!Number.isNaN(parseInt(pemData.serial.split(' ')[0], 10)) && pemData.serial.indexOf(':') < 0) {
          ...
          tbkSecurity.certSerial = tokens.reduce((acc, elem) => `${acc}${parseInt(`0x${elem}`, 8)}`, '');
        }
        console.debug(tbkSecurity.certSerial);
